    ...
    registerAdmin(req, res) {
        ...
        this.model.Admin.findOne({ userName: req.body.userName }, (err, Admin) => {
            ...
            if (Admin) {
                ...
                let newAdmin = new this.model.Admin({
                ...
                newAdmin.save(err => {
                    if (err) throw err;
                    let token = jwt.sign({ user_id: newAdmin._id }, config.secret, { expiresIn: '110h', });
                    this.model.Admin.findByIdAndUpdate(newAdmin.id, { token: token }, (err, result) => {
                        if (err) throw err
        ...
        this.model.Admin.findOne({ userName: req.body.userName }, (err, Admin) => {
            ...
            bcrypt.compare(req.body.password, Admin.password, (err, status) => {
                ...
                this.model.Admin.findByIdAndUpdate(Admin.id, { token: token }, (err, result) => {
                    if (err) throw err
                    this.model.Admin.findById(result.id, { token: 1, userName: 1, type: 1, image: 1, fullName: 1,accessLevel:1 }, (err, auth) => {
                        return res.json({ data: auth, success: true });
                    });
                });
            })

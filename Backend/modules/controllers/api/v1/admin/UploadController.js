...
module.exports = new class UploadController extends Controller {
    ...
    uploadImage(req, res) {
        if(req.file) { 
            res.json({
                message : 'تصویر با موفقیت آپلود شد',
               imagePath :`${config.uploadUrl}` + req.file.path.replace(/\\/g , '/'),
                data :req.file,
                success : true,
            })
        } else {
    ...
    uploadVideo(req, res) {
        if(req.file) {
            res.json({
                ...
                videoPath : `${config.uploadUrl}` + req.file.path.replace(/\\/g , '/'),
                data :req.file,
                success : true,
            })
        } else {
        ...
        var fs = require('fs');
        try {
            fs.unlink((`${config.deleteUrl}`+req.body.path).replace(/\\/gi, '/'), function (err) {
	if (err) { 
	       return res.json({
